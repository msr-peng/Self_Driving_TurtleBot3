...
def get_thresholded_image(img):
	...
	gray_threshold = 110
	g_condition = equ_gray < gray_threshold

	# set b channel threshold to isolate orange channel
	color_threshold = 150
	B = img[:,:,0]
	G = img[:,:,1]
	R = img[:,:,2]
	...
	combine_condition = g_condition | b_condition
	combine_condition = combine_condition*1
	
	# apply the region of interest mask
	mask = np.zeros_like(gray)
	region_of_interest_vertices = np.array([[0,height-1], [0, int(0.6*height)], 
											[width-1, int(0.6*height)],
											[width-1, height-1]], dtype=np.int32)
	cv2.fillPoly(mask, [region_of_interest_vertices], 1)
	thresholded = cv2.bitwise_and(combine_condition, combine_condition, mask=mask)
...
def measure_curvature(x_values):
	...
	fit_pix = np.polyfit(y_points, x_values, 3)
...
def get_line_predictions_single(non_zeros_x, non_zeros_y, coordinates, num_rows):
	...
	polyfit = np.polyfit(y, x, 3)
	y_points = np.linspace(0, num_rows-1, num_rows)

	x_predictions = polyfit[0]*y_points**3 + polyfit[1]*y_points**2 + polyfit[2]*y_points + polyfit[3]
...
def pipeline_final_single(img):
	...
	if left:
		x_predictions = polyfit_left[0]*non_zeros_y**3 + polyfit_left[1]*non_zeros_y**2 + polyfit_left[2]*non_zeros_y + polyfit_left[3]
		polyfit_right = None
	else:
		x_predictions = polyfit_right[0]*non_zeros_y**3 + polyfit_right[1]*non_zeros_y**2 + polyfit_right[2]*non_zeros_y + polyfit_right[3]
		polyfit_left = None
	coordinates = ((non_zeros_x >= x_predictions - margin) & (non_zeros_x <= x_predictions + margin)).nonzero()[0]
	x_predictions = get_line_predictions_single(non_zeros_x, non_zeros_y, coordinates, num_rows)
	x_predictions = np.asarray(x_predictions)
	out_img[non_zeros_y[coordinates], non_zeros_x[coordinates]] = [0, 255, 0]
	xm_per_pix = 17.5/32000

	curve_rad, polyfit = measure_curvature(x_predictions)
	lane_center = x_predictions[num_rows-1]

	if left:
		polyfit_left = polyfit
	else:
		polyfit_right = polyfit

	curvature_string = "Curvature: %.2f m" % curve_rad
	center_offset_pixels = lane_center - warped_size[0]/2
	center_offset_mtrs = xm_per_pix*center_offset_pixels
	offset_string = "Deviation: %.2f m" % center_offset_mtrs

	road = cv2.resize(out_img, (200, 120))
	img[:road.shape[0], :road.shape[1], :] = road
	result = img

	cv2.putText(result,curvature_string , (220, 40), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0,255,0), thickness=2)
	cv2.putText(result, offset_string, (220, 90), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0,255,0), thickness=2)

	return result, center_offset_mtrs, curve_rad
...
def pipeline_final(img):
	...
	if (polyfit_left is None) or (polyfit_right is None):
		...
		left_x_predictions, right_x_predictions = get_line_predictions(non_zeros_x, non_zeros_y, left_coordinates, right_coordinates, num_rows)
		############## Debug
		# rospy.loginfo("used former curve as indicator")
	...
	if bad_lines:
		...
		mean_difference = np.mean(right_x_predictions - left_x_predictions)
		running_mean_difference_between_lines = 0.7*running_mean_difference_between_lines + 0.3*mean_difference
	...
	if sum(left_coordinates) > 3000 and sum(right_coordinates) > 3000:
		left_curve_rad, polyfit_left = measure_curvature(left_x_predictions)
		right_curve_rad, polyfit_right = measure_curvature(right_x_predictions)
		average_curve_rad = (left_curve_rad + right_curve_rad)/2
		lane_center = (right_x_predictions[num_rows-1] + left_x_predictions[num_rows-1])/2
		
	elif sum(left_coordinates) <= 3000: # only right lane get detected
		...
		if right_line_fit[0] > 0: # judge the side of lane by its gradient's sign
			...
			offset = 175*(1 + right_line_fit[0]**2)**0.5
		average_curve_rad, polyfit_right = measure_curvature(right_x_predictions)
		# polyfit_left = None
		...
		if left_line_fit[0] > 0: # judge the side of lane by its gradient's sign
			...
			offset = 175*(1 + left_line_fit[0]**2)**0.5
		average_curve_rad, polyfit_left = measure_curvature(left_x_predictions)
		# polyfit_right = None
	...
	result = cv2.addWeighted(img, 1, unwarped, 1, 0)  
...
def update_which_follow(whichlane):
	global center
	global right
	global left

	right = whichlane.right
	left = whichlane.left
...
def publish_lane(image, args):
	...
	if center:
		...
		draw, deviation, curvature = pipeline_final_single(cv2_img)
...
def get_lane():
	...
	rospy.Subscriber(lane_topic, WhichLane, update_which_follow)
